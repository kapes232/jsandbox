...
class Particle {
  ...
  update(mouseX: number, mouseY: number) {
    ...
    const distance = Math.sqrt(
      Math.pow(mouseX - this.sprite.x, 2) + Math.pow(mouseY - this.sprite.y, 2)
    );
    if (distance < 50) {
...
export default class Main extends React.Component<{}, IState> {
  ...
  renderParticles = () => {
    ...
    const load = PIXI.loader.add(url).load(this.setUp);
  };
  ...
  componentDidMount() {
    ...
    PIXI.loader.onError.add(() =>
      setTimeout(this.setState({ hasError: true }))
    );
  ...
  render() {
    return (
      <div className="main">
        <div className="pixi-container" ref="pixi" />
        <div className="input-container">
          Image URL:
          <input type="text" ref="input" defaultValue={DEFAULT_IMG} />
          <button className="button" onClick={this.renderParticles}>
            Submit
          </button>
        </div>
        {this.state.hasError && (
          <p className="error">
            Sorry, that image cannot be loaded because its server doesn't allow
            it, plz try another one, like:
            http://davidguan.me/assets/136133c6ea2e29e457cea61fbbc06ad3.png
          </p>
        )}
        <p className="desc">
          Image without CORS enabled on its server won't load, you can test this
          application with images on{" "}
          <a href="http://imgur.com/search?q=mountain" target="_blank">
            imgur.com
          </a>
        </p>
        <p className="footer">
          Visit{" "}
          <a href="https://davidguan.me" target="_blank">
            davidguan.me
          </a>{" "}
